...
class GetExpensesByGroupNameUseCase(private val expenseRepository: ExpenseRepository) {
    suspend fun execute(token: String, groupName: String): Resource<SplitterApiResponse<List<Expense>>> {
        return expenseRepository.getExpensesByGroupName(token, groupName)
    }
}